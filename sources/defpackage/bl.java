...
public class bl<K, V> {
    ...
    public void putAll(bl<? extends K, ? extends V> blVar) {
        int i = blVar.mSize;
        ensureCapacity(this.mSize + i);
        if (this.mSize != 0) {
            for (int i2 = 0; i2 < i; i2++) {
                put(blVar.keyAt(i2), blVar.valueAt(i2));
            }
        } else if (i > 0) {
            System.arraycopy(blVar.mHashes, 0, this.mHashes, 0, i);
            System.arraycopy(blVar.mArray, 0, this.mArray, 0, i << 1);
            this.mSize = i;
        }
    }
        ...
        if (obj instanceof bl) {
            ...
            for (int i = 0; i < this.mSize; i++) {
                try {
                    K keyAt = keyAt(i);
                    V valueAt = valueAt(i);
                    Object obj2 = blVar.get(keyAt);
                    if (valueAt == null) {
                        if (obj2 != null || !blVar.containsKey(keyAt)) {
                            return false;
                        }
