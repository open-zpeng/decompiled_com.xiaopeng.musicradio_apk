...
public class UpdateSceneTask extends BaseTask {
    ...
    private void buildUpdateView(SoftReference<View> softReference) {
        ...
        SoftReference<bic> softReference2 = this.callback;
        ...
        bin buildView = buildView(softReference, list, list2, softReference2, list3, j, list4, list5, null, 0, isVuiLayoutLoadable, z, z ? null : this.elementChangedListener);
        ...
        while (i < this.elements.size()) {
            bin binVar = this.elements.get(i);
            bin vuiElementById2 = this.vuiSceneCache.getVuiElementById(this.sceneId, binVar.getId());
            if (vuiElementById2 == null || !binVar.equals(vuiElementById2)) {
                i++;
            } else {
                this.elements.remove(binVar);
            }
        }
    }
    ...
    private void handleUpdateElement() {
        ...
        if (!"user".equals(Build.TYPE) && LogUtils.getLogLevel() <= LogUtils.LOG_DEBUG_LEVEL) {
            String str = this.TAG;
            LogUtils.logDebug(str, "updateScene completed time:" + (System.currentTimeMillis() - this.time) + ListSignBean.COLLECTID_SUFFIX + VuiUtils.vuiUpdateSceneConvertToString(this.vuiScene));
        }
