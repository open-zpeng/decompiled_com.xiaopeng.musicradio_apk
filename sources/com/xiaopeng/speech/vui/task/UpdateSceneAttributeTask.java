...
public class UpdateSceneAttributeTask extends BaseTask {
    ...
    private void updateScene() {
        try {
            ...
            if (this.updateView != null) {
                ...
                bin buildElement = buildElement(sceneCache, this.updateView, currentTimeMillis);
                if (buildElement != null) {
                    arrayList.add(buildElement);
                }
            } else if (this.viewList != null) {
                for (int i = 0; i < this.viewList.size(); i++) {
                    bin buildElement2 = buildElement(sceneCache, this.viewList.get(i), currentTimeMillis);
    ...
    private String getElementId(SoftReference<View> softReference) {
        if (softReference == null || softReference.get() == null || !(softReference.get() instanceof bhz)) {
            return null;
        }
        ...
        if (TextUtils.isEmpty(str)) {
            ...
            String vuiElementId = ((bhz) softReference.get()).getVuiElementId();
    ...
    private bin buildElement(VuiSceneCache vuiSceneCache, SoftReference<View> softReference, long j) {
        bin onBuildVuiElement;
        String elementId = getElementId(softReference);
        if (TextUtils.isEmpty(elementId)) {
            return null;
        }
        bin vuiElementById = vuiSceneCache.getVuiElementById(this.sceneId, elementId);
        if (vuiElementById == null) {
            LogUtils.e(this.TAG, "缓存中没有此元素");
            return null;
        } else if (softReference == null || !(softReference.get() instanceof bic) || (onBuildVuiElement = ((bic) softReference.get()).onBuildVuiElement(elementId, this)) == null) {
            ...
            if (vuiProps != null) {
                if (bih.STATEFULBUTTON.a().equals(buildVuiElementAttr.getType())) {
            ...
            if (!bih.STATEFULBUTTON.a().equals(buildVuiElementAttr.getType()) && vuiElementById.getElements() != null && vuiElementById.getElements().size() > 0) {
