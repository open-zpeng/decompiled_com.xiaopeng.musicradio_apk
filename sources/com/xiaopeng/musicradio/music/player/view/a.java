package com.xiaopeng.musicradio.music.player.view;

import android.content.Context;
import android.text.TextUtils;
import com.google.protobuf.AbstractMessageLite;
import com.xiaopeng.musicradio.music.player.presenter.MusicPlayerPresenter;
import com.xiaopeng.musicradio.proto.Player;
import com.xiaopeng.musicradio.utils.be;
/* compiled from: BtMusicPlayerViewProxy.java */
/* loaded from: classes.dex */
public class a extends aeu<g, MusicPlayerPresenter> implements g {
    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void G_() {
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void H_() {
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void a(int i, boolean z) {
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void b(int i, boolean z) {
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void b(boolean z) {
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void h() {
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void l() {
    }

    public a(Context context, String str) {
        super(context, str);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // defpackage.aeu
    /* renamed from: a */
    public MusicPlayerPresenter g() {
        return new MusicPlayerPresenter();
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void b_(boolean z) {
        int i;
        if (z) {
            return;
        }
        if (!((MusicPlayerPresenter) this.c).q() || ((MusicPlayerPresenter) this.c).r()) {
            i = ((MusicPlayerPresenter) this.c).r() ? 3 : 2;
        } else {
            i = 1;
        }
        a("BtMusicPlayerViewProxy_player_btmusic_updatebtstatus", (AbstractMessageLite) Player.BtConnectStatus.newBuilder().a(i).build());
    }

    @Override // com.xiaopeng.musicradio.music.player.view.g
    public void a(boolean z, boolean z2) {
        a("BtMusicPlayerViewProxy_player_btmusic_updatelove", (AbstractMessageLite) Player.MediaLove.newBuilder().a(z).b(z2).build());
    }

    @Override // com.xiaopeng.musicradio.playercomponent.big.view.b
    public void b(String str, String str2) {
        a("BtMusicPlayerViewProxy_player_btmusic_updatetitle", (AbstractMessageLite) Player.TitleInfo.newBuilder().a(be.e(str)).b(be.e(str2)).build());
    }

    public void j() {
        if (com.xiaopeng.musicradio.model.bluetooth.a.i().n()) {
            com.xiaopeng.musicradio.model.bluetooth.a.i().r();
            com.xiaopeng.musicradio.model.bluetooth.a.i().play();
            return;
        }
        bcp.a();
    }

    @Override // com.xiaopeng.musicradio.playercomponent.big.view.b
    public void F_() {
        c_("SetDefault", "");
    }

    @Override // defpackage.aeu, defpackage.aev
    public void a(String str, byte[] bArr) {
        super.a(str, bArr);
        if (TextUtils.equals("clickLove", str)) {
            ((MusicPlayerPresenter) this.c).i();
        } else if (TextUtils.equals("clickBlue", str)) {
            j();
        } else if (TextUtils.equals("getCurrent", str)) {
            ((MusicPlayerPresenter) this.c).z_();
            ((MusicPlayerPresenter) this.c).c();
        }
    }
}
