...
public class VuiFloatingView extends FrameLayout {
    ...
    public VuiFloatingView(Context context, int i) {
        ...
        this.mType = i;
        this.mFloatingView = (ImageView) LayoutInflater.from(this.mContext).inflate(bip.i.vui_floating, this).findViewById(bip.g.floating_view);
    ...
    private void checkAnimatorDrawable() {
        if (VuiImageDecoderUtils.isSupportNight(this.mType)) {
            boolean d = acs.d(getContext());
            if (this.mIsNight != d || this.mNeedReLoadDrawable) {
                refreshAnimatorDrawable(VuiImageDecoderUtils.decoderImage(this.mContext, this.mType, d));
                XLogUtils.d(TAG, "checkAnimatorDrawable isNight " + d + " ,mNeedReLoadDrawable " + this.mNeedReLoadDrawable);
                this.mIsNight = d;
